...
## Recoverable allocator OOM warnings during the book test

At 20:13:00–20:13:03 (+03:00), the cold 190,422-token book request overlapped other production traffic. All four ranks logged failed allocation attempts for 382, 460 and 512 MiB, with only 106.25, 370.25 and 508.25 MiB free respectively. These were **non-fatal in this incident**: the book request, started at 20:12:51.933, completed normally at 20:13:20.254 with 1,105 output tokens and finish_reason=stop. The container remained on the same startup instance, and health checks succeeded. An HTTP 200 alone would not establish successful streaming completion; the saved completed response does.

The exact pinned PyTorch allocator emits this warning on a failed cudaMalloc attempt, before trying cached-block release and allocation retries. Thus an OOM warning is not necessarily a final torch.OutOfMemoryError. The observed completion is consistent with successful recovery. Exact tensor ownership and allocator retry counters were not captured, so the temporary-buffer attribution is based on timing rather than a captured allocation stack. A separate _gumbel_sample_kernel JIT event at 20:13:08 also contributed latency.

The GPU had little spare memory. Startup recorded 10.55 GiB of KV allocation and 2.12 GiB of CUDA graphs; it calculated that 8.28 GiB of KV would fit the requested 0.85 budget after accounting for those costs. gpu_memory_utilization is not a hard runtime VRAM ceiling. A low KV-cache usage percentage is occupancy within an already allocated pool, not a measure of free GPU memory.

No serving settings were changed in response to this isolated recovered event. If more headroom is needed, explicitly reducing the KV allocation toward 8 GiB would reserve roughly 2.5 GiB more space for temporary work, at the expense of aggregate cached-token/concurrent-context capacity. Any such change needs its own long-context and throughput validation; it is not part of the measured recipe. See [incident evidence](../measurements/2026-09-11-multimodal/allocator-pressure-review.json).
